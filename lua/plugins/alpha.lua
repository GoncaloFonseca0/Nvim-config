...
if vim.api.nvim_exec('echo argc()', true) == '0' then
  ...
  -- cria um grupo AlphaDoom com cor aproximada ao logo original
  vim.api.nvim_set_hl(0, 'AlphaDoom', {
    fg = '#A20000', -- tom escuro de vermelho
    bg = 'NONE',
    bold = true,
  }) -- 2) Novo header em ASCII art “DOOM”
  -- usa o primeiro cabeçalho em Headers (o seu DOOM ASCII)
  local header = {
    type = 'text',
    val = Headers[1],
    opts = {
  ...
  local buttons = {
    ...
    val = {
      ...
      button('g', '  Open Last Session', 'RainbowGreen', ':source $LOCALAPPDATA/nvim/auto-session.lua'),
